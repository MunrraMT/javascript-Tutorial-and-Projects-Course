...
document.querySelector('.lorem-form').addEventListener('submit', (e) => {
	e.preventDefault();
	paragraph(text, inputNumber.value);
});

function paragraph(item, paragraphs) {
	loremText.textContent = '';

	if (paragraphs <= 0) {
		const p = document.createElement('p');
		p.textContent += item[rand(0, text.length)];
...
maxMinParagraphs();
function maxMinParagraphs() {
	inputNumber.setAttribute('min', '0');
	inputNumber.setAttribute('max', text.length);
